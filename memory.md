...
---
	- NGINX内存池
    NGINX是高性能高并发服务器的典范，NGINX常用来作为HTTP和反向代理Web服务器，作为HTTP服务器，NGINX的工作模式是：接收一个client request，处理该request，然后吐出response。
    
    这样的工作模式非常适合用内存池优化动态内存分配，NGINX精巧的内存池设计也是NGINX优秀设计的一个典型案例。
    
    NGINX为每个连接创建一个内存池对象，在该处理该连接请求的过程中，所有的动态内存分配请求，都向内存池提交，返回结果后，再清理该连接的内存池，清理内存池不一定会把内存真正返回系统，会根据策略把分配的块缓存起来，提高内存复用。
    
    因为请求的处理过程都在一个线程内进行，所以该连接的内存池不需要处理线程同步，中途也不需要释放内存（返回结果后统一释放），避免了内存泄漏的问题，安全性也得到了保障。

    NGINX内存池的设计概要：
        - request处理过程中，所有动态内存分配，都向连接专属的内存池申请，免锁。
        - 大尺寸内存按需分配，走malloc/mmap，大块内存块用链表串起来。
        - 小尺寸内存移动游标分配，先通过malloc/mmap申请4K的一个内存页（批发），再在这一页里每次分一小块（零售），其实就是批发转零售模式。
        - 内存池记着当前页指针，下次分配先从当前页尝试，只在一页多次不满足分配请求才改当前页指针。

    NGINX内存池处理分配请求逻辑：    
        - 如果大于等于某个阈值，则被视为大块内存，大块内存分配直接调用标准C的malloc函数，内存池为大块内存维护一个单独的链表，用于统一释放，大块支持独立释放。
        - 如果小于某个阈值，会先判断当前页剩余的内存大小能否满足本次分配的尺寸要求；如果满足，则简单的移动游标，返回移动前的右边位置（地址）；不满足的话，再次通过底层接口分一内存页，并从该页分配一块满足本次请求。
        - 分配的内存页，也会通过链表串起来。
        - 假设当前页还剩500字节，但接下来的分配请求512字节，那么内存池会分配一个4K的内存页，从这个内存页划分出512字节返回，并把这个内存页串到页链表，但内存池下次分配请求可能依然从还有500字节剩余的内存页开始尝试。
        - 只有一个内存页在多次不满足分配尺寸要求后，才会修改内存池的当前页指针，这样做是为了减少内存浪费，如果多次尝试后，依然不满足，那么这个内存页的剩余空间大概率会比较小，这时候跳过它，也就合情合理了。

    - NGINX内存池小结：NGINX内存池，为每个连接配一个内存池，使得无锁成为可能，NGINX内存池专注于做好小内存的分配，大块的分配只是简单转交给malloc，它在处理当前内存页指针的细节上做的比较好，提升了内存利用率，跟一般内存池一样，通过舍弃中途释放能力，换取移动游标分配内存的高性能，另外它缓存了内存的策略本质上是牺牲空间换时间。
