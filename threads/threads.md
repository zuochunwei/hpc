# 线程的概念
线程代表进程内的一个执行上下文，每个线程有自己的寄存器值、调用栈、信号掩码、错误码（errno）、线程私有数据、执行流。

操作系统中，被调度执行的最小单位是线程，而不是进程。Linux kernel用任务（task）表示一个执行流，进程是通过共享存储空间对用户呈现的概念。
...
进程是资源分配的基本单位。同一进程内的多个线程共享地址空间和文件描述符，共享地址空间意味着进程的代码区域（函数）、全局变量、堆、栈都被进程内的多线程共享。
...
如果文件不多、又或者每个文件不大，那么运行这个程序，很快就会得到统计结果，否则，运行这个单线程程序可能要等很久才能返回结果。

重新审视这个程序会发现：函数`word_count_a_file()`只接受一个文件名，吐出从该文件计算出的局部结果，它不依赖于其他外部数据和逻辑，可以并发执行，所以，可以为每个文件启动一个单独的线程去运行`word_count_a_file()`，等到所有线程都执行完，再合并映射表得到最终结果。
