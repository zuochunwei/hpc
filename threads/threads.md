...
## 执行流
一个任务里被依次执行的指令，会形成一个指令序列，这个指令序列就是一个指令流，每个线程会有自己的执行流。
...
芯片设计领域，一个硬件线程通常指为执行指令序列而配套的硬件单元，我们知道一个CPU可能有多个核心，然后一个核心有可以开2个超线程，这2个超线程复用一些硬件，但基本上可以认为是独立的执行单元。
