...
那么，如何用好C++这个强大的语言工具去完成业务目标呢？

我的建议是“**通过人为限制去控制语言复杂度，把更多精力放到设计和领域。**”而这在各种流行的编码规范里都没有提到。只使用常用的语法特征和精心挑选的编程接口，即只使用标准C++的一个非常有限的子集，这样就能降低复杂度，同时又获得C++零成本抽象和语言结构直接映射到硬件的双核心能力。
