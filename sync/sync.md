# 什么是多线程同步？
同一进程内的多个线程会共享数据，对共享数据的并发访问会出现`race condition`，这个词的官方翻译是竞争条件，但`condition`翻译成条件令人困惑，特别是对初学者而言不够清晰明了，我看翻译软件`condition`有状况、状态、情况的含义，可能翻译成竞争状况更直白。

多线程同步是指：
- 协调多个线程对共享数据的访问，避免出现数据不一致的情况
- 协调各个事件的发生顺序，使多线程在某个点交汇并按预期步骤往前推进，比如某线程需要等另一个线程完成它的某项工作才能进行该线程的下一步工作。

# 为什么需要同步?
要掌握多线程同步，需先理解为什么需要多线程同步、哪些情况需要同步。

理解为什么要同步是多线程编程的关键，它甚至比掌握多线程同步机制更加重要；识别哪里需要同步是多线程编程的难点，只有能准确识别需要保护的数据、需要同步的点，再配合系统或语言提供的合适的同步机制，才能编写安全高效的多线程程序。
...
2. 如果2个线程，在执行自增`++x;`的时候，时间上没有完全错开。假设线程1在core1上执行，线程2在core2上执行，那么，可能的执行过程是这样的：
    - 首先，线程1把`x`读到core1的寄存器，线程2也把`x`的值加载到core2的寄存器，此时，两个core的寄存器里的值都是0。
    - 然后，线程1完成自增，更新寄存器里`x`的值的拷贝（0变1），线程2也完成自增，更新寄存器里`x`的值的拷贝（0变1）。
    - 然后，线程1将更新后的新值1写入变量`x`的内存位置。
    - 最后，线程2将更新后的新值1写入同一内存位置，变量`x`的最终值是1，不符合预期。

线程1和线程2在同一个core上交错执行，也有可能出现同样的情况，之所以会出现不符合预期的情况，主要是因为**加载+更新+保存**这3个步骤不能在一个内存周期内完成，多个线程对同一变量并发读写，不加同步的话会出现数据不一致（该例为终值既可能为1也可能为2）。
...
这个要求，其实就是要让多个线程互斥访问共享资源。即如果有一个线程正在访问某共享资源（临界资源），那么其他线程不能执行访问同一资源的代码，访问临界资源的代码叫临界代码。

如果其他线程想要访问同一资源，则它必须等待，直到那个线程访问完成，其他线程才能获得访问的机会。简言之：就是多线程对共享资源的访问**串行化**。

这好比高速公路上的汽车过检查站，假设高速公路有多车道，但检查站只有一个车道，多车道好比并发的多执行流，无论高速路上车道多少，过检查站的时候只能一辆车接着一辆车，依次鱼贯而过。
...
```
lock_guard是互斥量的一个`RAII`包装类，unique_lock除了会在析构函数自动解锁外，还支持主动unlock调用。
