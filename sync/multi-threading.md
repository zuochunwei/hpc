...
void thread2() {
    char new_msg[256] = "this is new msg, it's very looooooooooooooooooooooooooooooooooooooooooog";
    write_msg(new_msg, sizeof(new_msg))
}
```
如果线程1调用write_msg()，因为msg的长度是256字节，完成对256字节的内存写入需要多个内存周期，在线程1写入新消息期间，如果线程2读msg，则线程2可能在线程1写入数据过程中，读到不一致的数据（即可能读到"this is new msg"，而"it's very..."还没来得及写入），它不是完整的新消息，这可能不是我们想要的。

例子2：比如”用数组实现队列“，该队列通常会有一个元素的数组和2个游标，游标用于记录队首head和队尾tail的位置。

队列的写入接口，会先把元素添加到队尾，再移动队尾游标，但**添加元素+移动游标**这2个操作，不能在一个指令周期内完成，所以，读的线程也可能穿插进来，同样会有问题。

再比如一个结构体有多个成分（成员），如果这多个成分是关联的，如果完成了一个成分的写入，还没来得其他成分写入的时候，就被其他线程读到了，那也是有问题的。比如对于二叉搜索树的节点:
```c++
struct Node {
    struct Node *parent;
...
```
可能需要同时更新节点的这3个指针才应该被其他读线程访问，如果只是更新了部分指针成分，则可能出错。
