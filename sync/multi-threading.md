...
多个线程对同一数据的并发访问叫数据竞争，数据竞争不加控制，会出现数据不一致的情况，这种情况需要避免，协调多线程对共享数据并发访问的机制叫多线程同步，操作系统和编程语言提供多种多线程同步的手段。
...
再看看哪些情况需要同步，哪些情况不需要同步：
- 如果不存在数据共享，则不需要同步。不存在数据共享并非指不存在可以被多线程访问的数据，而是指每个线程不读写被其他线程读写的数据。
- 如果存在共享的数据，又分2种情况
    - 如果多线程对共享数据只读，那么不需要同步。
    - 如果多线程对共享数据读写，那么通常需要同步。注意是通常，而非必须，如果能够容忍这种数据竞争导致的问题，则同步也不是必须的。

### 两个线程读写同一个数据
例子1：假设有1个长度为256的字符数组表示的消息，write_msg()函数实现对msg的写操作，如下：
```c++
char msg[256];
...
void thread_b() {
    char new_msg[256] = "this is new msg content";
    write_msg(new_msg, sizeof(new_msg))
}
```
如果线程A调用write_msg()，因为msg的长度是256字节，完成对256字节的内存写入需要多个内存周期，这线程A写入新消息期间，如果线程B读msg，则线程B可能在线程A写入数据过程中，读到不一致的数据（即可能读到"this is new"，而"content"还没来得及写入），它不是完整的新消息，这种情况是需要避免的。

例子2：比如”用数组实现队列“，该队列通常会有一个元素的数组和2个游标，游标用于记录队首head和队尾tail的位置。

队列的写入接口，会先把元素添加到队尾，再移动队尾游标，但添加元素+移动游标这2个操作，不能在一个内存周期内完成，所以，读的线程也可以穿插进来，同样会有问题。

再比如一个结构体有多个成分，如果这多个成分是关联的，如果完成了一个成分的写入，还没来得其他成分写入的时候，就被其他线程读到了，那也是有问题的。比如对于二叉搜索树的节点:
```c++
struct Node {
    struct Node *parent;
...
```
可能需要同时更新节点的这3个指针才能被其他读线程访问，如果只是更新了部分指针成分，则会出错。

### 两个线程同时写一个数据
最简单的例子就是两个线程对一个整型变量`int x`同时做自增`++x`，假设x的初始值是0。
...
这3个步骤不能在一个内存周期内完成，两个线程同时执行自增会交错执行，会出现数据不一致：
1. 如果2个线程，先后执行自增，在时间上完成错开，那么x的终值是2。
2. 如果线程1把x（值为0）读到寄存器，然后，线程2也把x（值为0）读到寄存器；然后线程1完成自增，并将更新后的新值1写入内存；然后线程2做自增，因为线程2寄存器里的旧值是0，线程2对x自增后也是1，线程2再更新值到内存，那么x的终值是1。

可见，不加同步的话，2个线程对同一变量并发读写，会出现数据不一致。
