...
1. 如果2个线程，先后执行自增，在时间上完成错开，那么x的最终值是2，符合预期。但多线程并发并不能确保对一个变量的访问在时间上完全错开。
2. 如果2个线程，在执行自增（`++x;`）的时候，时间没有完全错开：
    ...
    - 最后，线程2将更新后的新值1写入内存，变量`x`的最终值是1，不符合预期。

之所以会出现不符合预期的情况，主要是因为“加载+更新+保存”这3个步骤不能在一个内存周期内完成，如果这3个步骤是不可分割的整体，便不会有问题，可见，多个线程对同一变量并发读写，不加同步的话会出现数据不一致。
